...
export default function Home() {
  ...
  const connectToFreighter = async () => {
    ...
    try {
      ...
      if (!conn.isConnected) {
        throw new Error('Freighter not detected. Install https://freighter.app then refresh.');
      }

      console.log('🚀 Requesting access - POPUP NOW');
      const result = await requestAccess();
      console.log('Result:', result);
      
      if (result.error || !result.address) {
        throw new Error(result.error?.message || 'User cancelled');
      }

      const pk = result.address;
      console.log('✅ Connected PK:', pk);

      // Network check AFTER popup success
      const net = await getNetwork();
      let networkWarning = '';
      if (net.network !== 'TESTNET') {
        networkWarning = ` ⚠️ On ${net.network}, recommend TESTNET for app.`;
        console.warn('Wrong network:', net.network);
      }
      ...
      setError(error.message || 'Failed. See F12 console. Common fixes: unlock Freighter, disable popup blocker, refresh.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-8 bg-gray-900 p-8 rounded-2xl shadow-2xl border border-gray-800">
        <div className="text-center">
          ...
          <p className="mt-3 text-sm text-gray-400">
            Secure Stellar wallet connection for instant payments
          </p>
        </div>
